...
class PQCircularProgress(qtw.QWidget):
    def __init__(
        self,
        value=0,
        progress_width=10,
    ...
    def add_shadow(self, enable):
        if enable:
            self.shadow = qtg.QGraphicsDropShadowEffect(self)
            self.shadow.setBlurRadius(15)
            self.shadow.setXOffset(0)
            self.shadow.setYOffset(0)
            self.shadow.setColor(qtc.qtg.QColor(0, 0, 0, 80))
    ...
    def paintEvent(self, e):
        ...
        paint = qtg.QPainter()
        paint.begin(self)
        paint.setRenderHint(qtg.QPainter.Antialiasing)  # remove pixelated edges
        ...
        if self.enable_text:
            ...
            paint.setPen(pen)
            paint.drawText(rect, qtc.Qt.AlignCenter, f"{self.value}{self.suffix}")
