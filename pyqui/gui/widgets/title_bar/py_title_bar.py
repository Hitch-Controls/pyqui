...
# ///////////////////////////////////////////////////////////////
class PyTitleBar(qtw.QWidget):
    # SIGNALS
    clicked = qtc.Signal(object)
    released = qtc.Signal(object)

    drag_start = qtc.Signal()
    drag_pos = qtc.Signal(qtc.QPoint)
    drag_stop = qtc.Signal()
    ...
    def __init__(
        ...
        # self.top_logo.setPixmap(svg_image(logo_image))
        self._drag_pos = None
        ...
        self.title_frame_layout.addWidget(self.top_logo)
        self.title_frame_layout.addWidget(self.div_1)
        self.title_frame_layout.addWidget(self.title_label)
        self.bg_layout.addWidget(self.title_frame)
        self.bg_layout.addWidget(self.div_2)
    ...
    def add_menus(self, parameters):
        if parameters != None and len(parameters) > 0:
            for parameter in parameters:
                _btn_icon = svg_icon(parameter["btn_icon"])
    ...
    def maximize_restore(self, e=None):
        ...
        def change_ui():
            if _is_maximized:
                ...
                self.maximize_restore_button.set_icon(svg_icon("icon_restore.svg"))
            else:
                ...
                self.maximize_restore_button.set_icon(svg_icon("icon_maximize.svg"))
        ...
        if self._parent.isMaximized():
            ...
            _is_maximized = True
            _old_size = qtc.QSize(self._parent.width(), self._parent.height())
    ...
    def setup_ui(self):
        # ADD MENU LAYOUT
        self.title_bar_layout = qtw.QVBoxLayout(self)
        ...
        # ADD BG LAYOUT
        self.bg_layout = qtw.QHBoxLayout(self.bg)
        ...
        self.title_frame_layout.setContentsMargins(10, 0, 0, 0)
        ...
        self.top_logo = qtw.QLabel(self)
        self.top_logo_layout = qtw.QHBoxLayout(self.top_logo)
        self.top_logo_layout.setContentsMargins(0, 0, 0, 0)

        if self._logo_image.casefold().endswith('.svg'):
            self.logo = QtSvgWidgets.QSvgWidget()
            self.logo.load(svg_image(self._logo_image))
        else:
            self.logo = qtw.QLabel()
            self.logo.setScaledContents(True)
            pix = qtg.QPixmap(self._logo_image)
            pix = pix.scaledToHeight(24)
            self.logo.setMaximumWidth(pix.width())
            self.top_logo.setMaximumWidth(pix.width())
            self.logo.setPixmap(pix)
        self.top_logo_layout.addWidget(self.logo, qtc.Qt.AlignCenter, qtc.Qt.AlignCenter)

        # TITLE LABEL
        self.title_label = qtw.QLabel(self)
        self.title_label.setObjectName("title_label")
        self.title_label.setAlignment(qtc.Qt.AlignVCenter)
        self.title_label.setStyleSheet(f'font: {self._title_size}pt "{self._font_family}"')

        # CUSTOM BUTTONS LAYOUT
        self.custom_buttons_layout = qtw.QHBoxLayout()
        ...
        self.minimize_button = PyTitleButton(
            ...
            text_foreground=self._text_foreground,
            radius=6,
            icon_path=svg_icon("icon_minimize.svg"),
        )
        ...
        self.maximize_restore_button = PyTitleButton(
            ...
            text_foreground=self._text_foreground,
            radius=6,
            icon_path=svg_icon("icon_maximize.svg"),
        )
        ...
        self.close_button = PyTitleButton(
            ...
            text_foreground=self._text_foreground,
            radius=6,
            icon_path=svg_icon("icon_close.svg"),
        )

        # ADD TO LAYOUT
        self.title_bar_layout.addWidget(self.bg)

    def drag_rect(self) -> qtc.QRect:
        cr = self.title_frame.childrenRect()
        cr.moveTo(self.mapToGlobal(qtc.QPoint(cr.x(), cr.y())))
        cr.adjust(-10, 0, 10, 0)
        return cr

    def mousePressEvent(self, event: qtg.QMouseEvent):
        if self.drag_rect().contains(event.globalPos()):
    ...
    def mouseMoveEvent(self, event):
        if self._drag_pos is not None:
            p = event.globalPos() - self._drag_pos
            self.drag_pos.emit(p)
            self._drag_pos = event.globalPos()
