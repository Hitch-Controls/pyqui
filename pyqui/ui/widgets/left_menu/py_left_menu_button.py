...
# PROJECT MADE WITH: qtc.Qt Designer and PySide6
# V: 1.0.0
#
...
#
# There are limitations on qtc.Qt licenses if you want to use your products
...
# CUSTOM LEFT MENU
# ///////////////////////////////////////////////////////////////
class PyLeftMenuButton(qtw.QPushButton):
    def __init__(
        self,
        app_parent,
        text,
        ...
        self._icon_active_menu = svg_icon(icon_active_menu)
    ...
    def paintEvent(self, event):
        ...
        # RECTANGLES
        rect = qtc.QRect(4, 5, self.width(), self.height() - 10)
        rect_inside = qtc.QRect(4, 5, self.width() - 8, self.height() - 10)
        rect_icon = qtc.QRect(0, 0, 50, self.height())
        rect_blue = qtc.QRect(4, 5, 20, self.height() - 10)
        rect_inside_active = qtc.QRect(7, 5, self.width(), self.height() - 10)
        rect_text = qtc.QRect(45, 0, self.width() - 50, self.height())

        if self._is_active:
            # DRAW BG BLUE
            p.setBrush(qtg.QColor(self._context_color))
            p.drawRoundedRect(rect_blue, 8, 8)

            # BG INSIDE
            p.setBrush(qtg.QColor(self._bg_one))
            ...
            # DRAW TEXT
            p.setPen(qtg.QColor(self._set_text_active))
            p.drawText(rect_text, qtc.Qt.AlignVCenter, self.text())
            ...
            # DRAW BG BLUE
            p.setBrush(qtg.QColor(self._dark_four))
            p.drawRoundedRect(rect_blue, 8, 8)
        ...
        # NORMAL BG
            ...
            if self._is_toggle_active:
                # BG INSIDE
                p.setBrush(qtg.QColor(self._dark_three))
                p.drawRoundedRect(rect_inside, 8, 8)

                # DRAW TEXT
                p.setPen(qtg.QColor(self._set_text_foreground))
                p.drawText(rect_text, qtc.Qt.AlignVCenter, self.text())
                ...
                # BG INSIDE
                p.setBrush(qtg.QColor(self._set_bg_color))
                p.drawRoundedRect(rect_inside, 8, 8)
    ...
    def change_style(self, event):
        if event == qtc.QEvent.Enter:
            if not self._is_active:
                self._set_icon_color = self._icon_color_hover
                self._set_bg_color = self._dark_three
            self.repaint()
        elif event == qtc.QEvent.Leave:
            if not self._is_active:
                self._set_icon_color = self._icon_color
                self._set_bg_color = self._dark_one
            self.repaint()
        elif event == qtc.QEvent.MouseButtonPress:
            if not self._is_active:
                self._set_icon_color = self._context_color
                self._set_bg_color = self._dark_four
            self.repaint()
        elif event == qtc.QEvent.MouseButtonRelease:
            if not self._is_active:
                self._set_icon_color = self._icon_color_hover
                self._set_bg_color = self._dark_three
            self.repaint()
    ...
    def enterEvent(self, event):
        self.change_style(qtc.QEvent.Enter)
    ...
    def leaveEvent(self, event):
        self.change_style(qtc.QEvent.Leave)
        self.tooltip.hide()
    ...
    def mousePressEvent(self, event):
        if event.button() == qtc.Qt.LeftButton:
            self.change_style(qtc.QEvent.MouseButtonPress)
            self.tooltip.hide()
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == qtc.Qt.LeftButton:
            self.change_style(qtc.QEvent.MouseButtonRelease)
    ...
    def move_tooltip(self):
        ...
        gp = self.mapToGlobal(qtc.QPoint(0, 0))
...
class _ToolTip(qtw.QLabel):
    ...
    def __init__(self, parent, tooltip, dark_one, context_color, text_foreground):
        ...
        self.shadow.setColor(qtg.QColor(0, 0, 0, 80))
